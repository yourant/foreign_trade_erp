...
import com.fy.erp.modules.oa.dao.TestAuditDao;
...
import com.fy.erp.modules.oa.entity.TestAudit;
...
public class TestAuditService extends CrudService<TestAuditDao, TestAudit> {
    ...
    public int save(TestAudit testAudit) {
        int count = 0;
        // 申请发起
        if (StringUtils.isBlank(testAudit.getId())) {
            testAudit.preInsert();
            count = dao.insert(testAudit);
