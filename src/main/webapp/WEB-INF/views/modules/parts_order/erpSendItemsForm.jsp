...
<ul class="nav nav-tabs my-nav-tabs">
	<li class="active"><a>录入配件发货信息</a></li>
</ul><br/>
<div class="my-container">
	<form:form id="inputForm" modelAttribute="erpPartsOrder" action="${ctx}/erp/erpPartsOrder/save" method="post" class="form-horizontal">
				...
				<div>
						...
						<tr>
							<td colspan="6">
								...
								<c:if test="${fns:contains(erpPartsOrder.status,'1,2,4')}">
									<a onclick="showItemsDialog('${ctx}/erp/erpCarParts/partsList?partsOrderId=${erpPartsOrder.id}','配件列表')" class="btn">
										添加配件
									</a>
								</c:if>
							</td>
							<td>
								总价：<c:if test="${!fns:contains(erpPartsOrder.status,'1,2,3,5,7')}">${erpPartsOrder.price}</c:if><c:if test="${fns:contains(erpPartsOrder.status,'1,2,3,5,7')}"><input name="price" id="price" <c:if test="${!fns:contains(erpPartsOrder.status,'3,5,7')}">disabled="disabled" readonly="readonly"</c:if>></c:if>
							</td>
						</tr>
						</tfoot>
					</table>
					<script type="text/template" id="erpSendItemsTpl">//<!--
						<tr id="erpSendItemsList{{idx}}" class="sendItems-lists" data-vinid="{{row.erpVin.id}}" data-dockerid="{{row.erpDocker.id}}">
							...
							<td>
								<input id="erpSendItemsList{{idx}}_count" name="erpSendItemsList[{{idx}}].count" type="number" min="0" value="{{row.count}}" maxlength="11" class="input-small digits sum" <c:if test="${fns:contains(erpPartsOrder.status,'6,8,10')}">disabled="true" readonly="true" </c:if>/>
							</td>
							<td>
								<p class="sum_price"></p>
							</td>
							<td>
								<textarea id="erpSendItemsList{{idx}}_remarks" name="erpSendItemsList[{{idx}}].remarks" rows="4" maxlength="255" class="input-small " <c:if test="${fns:contains(erpPartsOrder.status,'6,8,10')}">disabled="true" readonly="true" </c:if>>{{row.remarks}}</textarea>
							</td>
		...
		<div class="form-actions my-form-actions">
			...
			<c:if test="${erpPartsOrder.act.taskId != null && erpPartsOrder.act.taskId != ''}">
