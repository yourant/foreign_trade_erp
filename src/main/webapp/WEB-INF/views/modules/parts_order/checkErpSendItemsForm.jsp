...
<ul class="nav nav-tabs my-nav-tabs">
	<li class="active"><a>审批配件订单${fns:contains(erpPartsOrder.status,'2')?'配件信息':fns:contains(erpPartsOrder.status,'5')?'收款信息':'发货信息'}</a></li>
</ul><br/>
<div class="my-container">
	<form:form id="inputForm" modelAttribute="erpPartsOrder" action="${ctx}/erp/erpPartsOrder/save" method="post" class="form-horizontal">
		...
		<c:if test="${fns:contains(erpPartsOrder.status,'8')}">
		<div class="control-group">
				...
				<div>
					<table class="table table-striped table-bordered table-condensed">
						...
						<tr>
							...
							<td colspan="2">
								总价：<c:if test="${fns:contains(erpPartsOrder.status,'1,2,3,5,7,8')}">${erpPartsOrder.price}</c:if><c:if test="${!fns:contains(erpPartsOrder.status,'1,2,3,5,7,8')}"><input name="price" id="price" disabled="disabled" readonly="readonly"></c:if>
							</td>
						</tr>
						</tfoot>
					</table>
		...
		<div class="form-actions my-form-actions">
			<c:if test="${erpPartsOrder.act.taskId != null && erpPartsOrder.act.taskId != ''}">
				<shiro:hasPermission name="erp:erpPartsOrder:oa:checkSendItemView">
					<input class="btn btn-success" type="submit" value="审批${fns:contains(erpPartsOrder.status,'2')?'配件信息':fns:contains(erpPartsOrder.status,'5')?'收款信息':'发货信息'}" onclick="$('#ipt_draft').val(0);"/>&nbsp;
				</shiro:hasPermission>
			</c:if>
